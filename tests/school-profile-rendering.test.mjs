...
for (const [badges,a,b] of [ [['A',null],['A'],[null]], [['B',null],[null],['B']], [['A','B'],['A'],['B']], [['A'],['A'],[]], [['B'],[],['B']], [[null],[null],[null]] ]) {
  test(`A/B counterpart ${badges}`,()=>{
    const day=badges.map(badge=>({...lesson,badge}));
    assert.deepEqual(filterDay(day,'A',kfg).map(l=>l.badge),a);
    assert.deepEqual(filterDay(day,'B',kfg).map(l=>l.badge),b);
    assert.equal(filterDay(day,'',kfg),day);
  });
}
...
test('teacher lookup case insensitive; source unchanged; cancellation abbreviation',()=>{
  const before=JSON.stringify(hass);
  const result=ctx.lesson(lesson,new Date(2026,8,7),hass);
  assert.equal(result.displayTeacher,'Herr Beispiel');
  assert.equal(result.cancelled,true);
  assert.equal(result.changeClass,'change-entfall');
  assert.equal(JSON.stringify(hass),before);
...
test('original subject matching and resolved replacement',()=>{
  ...
  h.states['sensor.vertretungsplan_7n'].attributes.entries=[{klasse:'7n',datum:'2026-09-07',stunde:'1',fach_original:'M',fach:'E',vertreter:'drg',art:'Vertr',raum:'200'}];
  ...
  assert.equal(result.displaySubject,'Englisch');
  assert.equal(result.displayTeacher,'Frau Test');
  assert.equal(result.originalSubject,'Mathematik');
  assert.equal(result.room,'200');
  assert.equal(ctx.lesson(lesson,new Date(2026,8,8),h).changeLabel,undefined);
});

test('calendar teacher details and escaping',()=>{
  const c={_school:ctx,_hass:hass};
  assert.equal(schoolDescription(c,'Lehrer: her\nThema: her'),'Lehrer: Herr Beispiel\nThema: her');
...
test('async profile loading, opt-out, stale load and unknown profiles',async()=>{
  const C=schoolCard(Base),c=new C();
  c.setConfig({'school-profile':'kfg'});c.hass=hass;assert.equal(c.renders,undefined);await c._schoolReady;assert.ok(c._school);assert.equal(c.renders,1);
  c.setConfig({'school-profile':false});assert.equal(c._school,null);assert.equal(c._hass,hass);
  c.setConfig({'school-profile':'kfg'});c.setConfig({'school-profile':false});await c._schoolReady;assert.equal(c._school,null);
  c.setConfig({'school-profile':'does-not-exist'});await c._schoolReady;assert.match(c.shadowRoot.innerHTML,/konnte nicht geladen/);
  assert.throws(()=>c.setConfig({'school-profile':'../kfg'}));
});

test('teacher updates invalidate stable entity render cache',async()=>{
  const C=schoolCard(Base),c=new C();
  c.setConfig({'school-profile':'kfg'});await c._schoolReady;c.hass=hass;c._renderedEntity=timetable;c.hass=hass;assert.equal(c._renderedEntity,timetable);
  c.hass={states:{...hass.states,'sensor.kfg_kollegium':{attributes:{lehrer:{Her:'New name'}}}}};
  assert.equal(c._renderedEntity,null);
});
...
for (const name of ['stundenplan-card','stundenplan-tag-card','stundenplan-grid-card']) {
  test(`normal renderer with optional school profile: ${name}`,async(t)=>{
    ...
    await import(`../custom_components/sph/static/sph-${name}.js?v=0.6.0`);
    const h=structuredClone(hass);
    ...
    const Card=registry.get(`sph-${name}`),c=new Card();
    c.setConfig({entity:timetable.entity_id,'school-profile':'kfg'});c.hass=h;await c._schoolReady;
    assert.match(c.shadowRoot.innerHTML,/Herr Beispiel/);assert.doesNotMatch(c.shadowRoot.innerHTML,/Fallback Fach/);
    c.setConfig({entity:timetable.entity_id,'school-profile':false});
